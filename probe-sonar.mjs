...
await page.goto('https://sonarqube.void42.internal/sessions/new', { waitUntil: 'networkidle' });
await page.waitForTimeout(3000);
const links = await page.evaluate(() => Array.from(document.querySelectorAll('a, button')).map(e => `${e.tagName} text="${(e.innerText||'').trim().slice(0,40)}" href="${e.href||''}"`).filter(s => s.length > 30));
